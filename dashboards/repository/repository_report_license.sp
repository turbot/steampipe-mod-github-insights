...
query "repository_other_license_count" {
  sql = <<-EOQ
    ...
    and
      license_info ->> 'key' not in ('lgpl-3.0','lgpl-2.1','mpl-2.0','epl-2.0','osl-3.0','eupl-1.2','gpl-3.0','gpl-2.0','agpl-3.0','agpl-2.0','cc-by-sa-4.0','apsl','apache-2.0','mit','bsd-3','bsd-2','bsd-3-clause','bsd2-clause', 'cc-by-4.0', 'wtfpl', 'ms-pl', 'unlicensed')
    group by
      license_info;
  EOQ
}
...
query "repository_license_table" {
  sql = <<-EOQ
    select
      ...
      case
        when (license_info ->> 'key' in ('lgpl-3.0','lgpl-2.1','mpl-2.0','epl-2.0','osl-3.0','eupl-1.2')) then 'weak copyleft'
